...
// ---------------------------------------------------------------------------
// Load the prelude (core+mini) and webaudio, silencing libraries' load chatter.
const { strudel, webaudio } = await (async () => {
  const saved = [console.log, console.warn, console.error];
  console.log = console.warn = console.error = () => {};
  try {
    return {
      strudel: await import('../strudel.js'),
      webaudio: await import('@strudel/webaudio'),
    };
  } finally {
...
// `Pattern` is declared as an interface below; don't also emit it as a function.
// Skip names that would clash with built-in globals (we run with lib esnext, so
// the set is small — mostly capitalized constructors handled by the filter).
const GLOBAL_SKIP = new Set(['Pattern', 'globalThis']);
...
// Audio setup API from @strudel/webaudio (samples, registerSound,
// getAudioContext, …) — usable in setup.js. These don't return Patterns, so
// they're typed `=> any`. Skip names the core prelude already provides.
const coreNames = new Set([...exportedFns, ...exportedConsts]);
const webaudioFns = Object.keys(webaudio)
  .filter(
    (k) => isFn(webaudio[k]) && !k.startsWith('_') && !GLOBAL_SKIP.has(k) && !coreNames.has(k),
  )
  .sort();
...
for (const pkg of ['@strudel/core', '@strudel/mini', '@strudel/webaudio']) {
...
const fnDecls = exportedFns
  .filter((fn) => !GLOBAL_SKIP.has(fn))
  .map((fn) => {
    if (docs.has(fn)) documentedCount.fns++;
    return `${jsdocComment(fn)}declare function ${fn}(...args: any[]): Pattern;`;
  })
  .join('\n\n');

const constDecls = exportedConsts
  .filter((c) => !GLOBAL_SKIP.has(c))
  .map((c) => {
    const isPattern = strudel[c] instanceof Pattern;
    return `${jsdocComment(c)}declare const ${c}: ${isPattern ? 'Pattern' : 'any'};`;
  })
  .join('\n\n');

const webaudioDecls = webaudioFns
  .map((fn) => `${jsdocComment(fn)}declare function ${fn}(...args: any[]): any;`)
  .join('\n\n');

// Transport functions are created inside repl() (not module exports), and caiman
// exposes them as globals in the player. Hand-listed here so they autocomplete.
const transportDecls = [
...
const header = `// AUTO-GENERATED by framework/scripts/gen-types.js — do not edit by hand.
...
const outFile = join(root, 'strudel.globals.d.ts');
...
console.log(`  ${webaudioFns.length} webaudio setup functions (samples, registerSound, …)`);
